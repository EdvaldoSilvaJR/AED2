...
public class dados3 
{
	String nome;
	long totalpop;
	static String arq = "residentes.csv";
	...
	public void Imprime()
	{
		int i;
		double porc;
		double aux=valores[tamanho-2];
		for(i=0;i<total;i++)
			System.out.println("REGIAO " +estados.get(i)+" POPULAÇÃO " + valores[i] );
		for( i=0;i<total;i++)
		{
			porc=valores[i]/aux;
			porc=porc*100;
			int teste=(int) porc;
			System.out.println("TESTANDO PORC "+teste);
		}
	}
	...
	{
		double porc,aux=valores[tamanho-2];
		...
		for(int i=0;i<total-1;i++)
		{
			...
			System.out.println(pie.getValue(i));
		}
		JFreeChart grafico = ChartFactory.createPieChart3D("População Residente",pie,false,true,true);
		grafico.setBackgroundPaint(Color.black);
		grafico.getTitle().setPaint(Color.white);
		ChartFrame frame1 = new ChartFrame("Grafico população residente",grafico);
		frame1.setSize(1366,768);
		RefineryUtilities.centerFrameOnScreen(frame1);
		frame1.setVisible(true);
		
		ChartUtilities.saveChartAsJPEG(new File("C://Users//Sanada//Desktop/grafico3.jpeg"), grafico, 1366, 768);
	}
